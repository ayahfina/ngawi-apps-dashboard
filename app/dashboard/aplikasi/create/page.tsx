...
export default function CreateAplikasiPage() {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        toast.error("Gagal memuat data untuk dropdown")
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || "Gagal membuat aplikasi")
      }
    } catch (error) {
      console.error("Error creating application:", error)
      toast.error("Terjadi kesalahan saat membuat aplikasi")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Tambah Aplikasi Baru</h1>
          <p className="text-muted-foreground">
            Tambahkan aplikasi baru ke dalam sistem
          </p>
        </div>
      </div>
